...
function CourseDetail(props) {
 ...
 const {addToCart,showHideCart,cartItems} =useContext(CartContext)
 const navigate=useNavigate()


const handleClick=(idus,idcourse)=>{
  addToCart(props.e);
  axios.post("http://127.0.0.1:5173/api/learn/add",{iduser:idus,idcourse:idcourse}).then((res)=>{
    console.log(res);
...
const handle=()=>{
  ...
  return (
    <div>
       {console.log("zz",cartItems)}
   <div className="card">
  <img src={props.e.thumbnail} className="card-img"/>
...
<span className="text-currency">&nbsp;TND</span>
  <div className="card-button">
    <svg className="svg-icon" onClick={()=>{handleClick(idus.idusers,props.e.idcourses)}} >
...
<button className='button-explore' onClick={handle}>
  <span> Explore !
  </span>
</button>
</div>
